...
public class ParserTest
{
    ...
    private static class ParserTestCase
    {
        ...
        public static void test(String input, String method)
    ...
    {
        // The code below runs all single level nestings of the form factor
        // strongOp factor
        // Multi level
    ...
    {
        String[] elems = {"a * b & gamma / beta", "truthy & falsey * screenwidth % screenheight"};
    ...
    {
        String[] terms = {"(a * b & gamma / beta) + (screenwidth % screenheight)"};
    ...
    {
        String[] exprs = {"(abs < 3)", "(abc * 123 / true) == g",
    ...
    {
        String[] args = {"123(", "(3)", "((2352))", "((((3))))", "(3,5)",
            "((abc * 123 / true) == g, " + "(123 % screenwidth + screenwidth / screenheight) != 156 / a, "
                + "(123 % true + screen) >= 15598826 / a)"};
    ...
    {
        String[] args = {"abc_ident", "blur(3,4)", "gray(0)", "convolve(1, 0, 1)", "move(0,46)", "xloc;", "yloc;",
            "width;", "height;"};
    ...
    {
        String[] chains = {"a -> b", "a -> blur(3) -> convolve(1,2,3) -> xloc;", "width -> scale(2) -> a"};
    ...
    {
        String[] decs = {"integer aba_a", "boolean f", "image img", "frame fr"};
    ...
    {
        String[] paramDecs = {"integer aba_a", "boolean f", "file something_txt", "url users_google_com"};
    ...
    {
        String[] stmts = {"sleep 10;", "if(i < 3) {i <- u + 1;}", "while(i < 40) { if(i % 2 == 0) { i <- i / 2;} }",
    ...
    {
        String[] blocks = {"{}", "{integer i \n boolean b\n}", "{sleep 10;}", "{integer i while(true) {  }}",
            "{integer i while(true) { sleep 10; }}", "{integer i while(true) { move(3) -> height; }}",
            "{i <- i + 1;}",
    ...
    {
        String[] programs = {"null_program {}", "iden {integer i \n boolean b\n}", "sleeper_cell {sleep 10;}",
            "loop_de_loop {integer i while(true) {  }}",
            "an_actual_program_for_testing_collatz_conjecture " + "integer i, file img {" + "i <- 1; "
                + "img <- 0; " + "while(true) { " + "   if(i % 2 == 0) { i <- i / 2;} "
                + "   if(i % 2 == 1) {i <- 3*i + 1;} " + "   img -> move(0,i) -> blur(i/255) -> move(i, 0);}}",
